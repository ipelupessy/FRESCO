...
base1=[ lambda x: x**0, lambda x: x, lambda x: x**2,lambda x: x**3,lambda x : x**4]
...
class ColorConverter(object):
    def __init__(self,source, target, base=None,lmin=None,lmax=None,N=1000):
        if base is None:
          base=default_base
        if len(source)>len(base):
          raise Exception("provide enough base functions")
        if lmin is None:
          lmin=min( [min(x['wavelength']) for x in source]+ \
                    [min(x['wavelength']) for x in target] )
        if lmax is None:
          lmax=max( [max(x['wavelength']) for x in source]+ \
        ...
        for j,src in enumerate(source):
          xp=src['wavelength']
          fp=src['throughput']
          f=numpy.interp(larray.number,xp=xp.value_in(larray.unit),fp=fp,left=0.,right=0.)
          for i,b in enumerate(self.base):  
            bint=f*self.fbase(b)(larray)
            Amatrix[j,i]=numpy.trapz(bint,x=larray.number)#/numpy.trapz(f,x=larray.number)
        ...
        for j,src in enumerate(target):
          ...
          for i,b in enumerate(self.base):  
            bint=f*self.fbase(b)(larray)
            Bmatrix[j,i]=numpy.trapz(bint,x=larray.number)#/numpy.trapz(f,x=larray.number)
...
class XYZ_to_sRGB_linear(object):
    def __init__(self):
      self.conversion_matrix=numpy.array( [[3.2406,-1.5372,-0.4986],[-0.9689,1.8758,0.0415],[0.0557,-0.2040,1.0570]])
    def convert(self,x):      
        return self.conversion_matrix.dot(x)
